...
For this modeification, we need to use a fuzzy intersection instead of a hard one. Thus, we need to choose another method to combine significant proteins we got from multiple testing and random forest. I'd like to use the unique funtion to get all significant proteins generated by those two methods. Let's see how the result is affected.
...
By comparing the result with the original one, we can find that this modificition get a lower sensiticity and a higher specificity, which means it is better at identifing people with ASD and worse at distinguishing healthy people. Moreover, by modifing the intersection method, the accuracy remains the same but the roc_auc becomes higher. It means that this modificition makes the model performs better than before.
